...
def objective(logf, mu, lSig, g_mu, g_lSig, g_lmb, logfg, n_samples, allow_negative=False):
  ...
  lgh = -np.inf if g_lmb.shape[0] == 0 else logsumexp(np.log(np.maximum(g_lmb, 1e-64)) + log_sqrt_pair_integral(mu, lSig, g_mu, g_lSig))
  #get samples from h
  std_samples = np.random.randn(n_samples, d)
  h_samples = mu+np.exp(0.5*lSig)*std_samples
  lf = logf(h_samples)
  lh = (0.5*mvnlogpdf(h_samples, mu[np.newaxis, :], lSig[np.newaxis, :])).flatten()
  ln = np.log(n_samples)
  ...
  #if g_lmb.shape[0] > 0:
  #  log_denom = 0.5*logsumexp( np.array( [0., 2.*logsumexp(np.log(np.maximum(g_lmb, 1e-64)) + log_sqrt_pair_integral(mu, lSig, g_mu, g_lSig))] ), b=np.array([1., -1.]))
  #else:
  #  log_denom = 0.
  ...
  #lh = (0.5*mvnlogpdf(h_samples, mu[np.newaxis, :], lSig[np.newaxis, :])).flatten()
  #lf = logf(h_samples)
  #lg = logg(h_samples, g_mu, g_lSig, g_lmb) + logfg
  #ln = np.log(n_samples)


  ##if g_mu.shape[0] > 0:
  ##  print('lf: ' + str(lf[:5]))
  ##  print('ld: ' + str(log_denom))
  ##  print('lh: ' + str(lh[:5]))
  ##  print('lg: ' + str(lg[:5]))

  #lf_num = logsumexp(lf - lh - ln)
  #lg_num = logsumexp(lg - lh - ln)
    ...
    try:
      ...
      for n in range(n_init):
        if i==0:
          ...
          mu0 = g_mu[k,:] + np.random.randn(d)*4*np.exp(g_lSig[k,:])
          lSig = np.random.randn(d) + g_lSig[k,:] 
...
#    
#    #plot UBVI
#    Y = 2*logg(X, g_mu, g_lSig, g_lmb).reshape(500,500)
#    Y -= Y.max()
