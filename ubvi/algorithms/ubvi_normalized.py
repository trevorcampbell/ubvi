...
def objective(logf, mu, L, g_mu, g_Sig, g_Siginv, g_lmb, logfg, n_samples):
  ...
  lf = logf(h_samples)
  lg = logg(h_samples, g_mu, g_Sig, g_Siginv, g_lmb)
  ...
  lh_num = -np.inf if lhg == -np.inf else logsumexp(lhg+lg+lf-2.*lh)

  #pass through 1/(1-log x) if positive,  -1/(1-log(-x)) if negative
  if lf_num > lh_num:
    logobj = lf_num - lf_denom - lh_denom + np.log(1.-np.exp(lh_num-lf_num))
    assert logobj < 0., logobj
    return 1./(1.-logobj)
  else:
    logobj = lh_num - lf_denom - lh_denom + np.log(1.-np.exp(lf_num-lh_num))
    assert logobj < 0., logobj
    return -1./(1.-logobj)
...
def ubvi(logf, N, d, n_samples, n_logfg_samples, adam_learning_rate= lambda itr : 1./(1.+itr), adam_num_iters=1000, print_every=10, n_init=1):
  ...
  for i in range(N):
    ...
    obj = lambda x, itr : -objective(logf, x[:d], x[d:].reshape((d,d)), np.atleast_2d(g_mu[:i, :]), np.atleast_2d(g_Sig[:i, :, :]), np.atleast_2d(g_Siginv[:i,:,:]), g_lmb[:i], logfgsum, n_samples)
    grd = grad(obj)
