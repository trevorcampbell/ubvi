...
for i in range(len(ubvis)):
    ...
    fkl_ubvi[i,:] = kl_estimate(mus, Sigs, wts, logp, np.random.standard_cauchy, direction='forward')
    rkl_ubvi[i,:] = kl_estimate(mus, Sigs, wts, logp, np.random.standard_cauchy, direction='reverse')
    
    mus = bbvis[i]['mus']
    Sigs = bbvis[i]['Sigs']
    wts = bbvis[i]['weights']
    cput_bbvi[i,:] = bbvis[i]['cputs']
    fkl_bbvi[i,:] = kl_estimate(mus, Sigs, wts, logp, np.random.standard_cauchy, direction='forward')
    rkl_bbvi[i,:] = kl_estimate(mus, Sigs, wts, logp, np.random.standard_cauchy, direction='reverse')

    mus = bbvi2s[i]['mus']
    Sigs = bbvi2s[i]['Sigs']
    wts = bbvi2s[i]['weights']
    cput_bbvi2[i,:] = bbvi2s[i]['cputs']
    fkl_bbvi2[i,:] = kl_estimate(mus, Sigs, wts, logp, np.random.standard_cauchy, direction='forward')
    rkl_bbvi2[i,:] = kl_estimate(mus, Sigs, wts, logp, np.random.standard_cauchy, direction='reverse')
...
#plot BVIbad
lq = mixture_logpdf(X[:, np.newaxis], b2_mu, b2_Sig, b2_wt[-1])
fig.line(X, np.exp(lq), line_width=6.5, color=pal[2], legend='BVI70')
#plot BVIgood
lq = mixture_logpdf(X[:, np.newaxis], b_mu, b_Sig, b_wt[-1])
fig.line(X, np.exp(lq), line_width=6.5, color=pal[1], legend='BVI1')
#plot UBVI
lq = mixture_logpdf(X[:, np.newaxis], u_mu, u_Sig, u_wt[-1])
